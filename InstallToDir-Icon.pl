    ...
    if ((-d "$dirname/$filename") ) {
      ...
      if (!(defined($commander)) || $commander eq "1" || $commander eq "2") {
        ...
        if ($? == -1) {
          ...
          printf "ERROR: Unknown. AMAI not have installed correctly. Adding item icons:%d\n", $? >> 8;
        }
        ...
        if ($? == -1) {
          ...
          printf "ERROR: Unknown. AMAI not have installed correctly. Adding input files:%d\n", $? >> 8;
        }
        system "MPQEditor a \"$dirname/$filename\" Icons\\war3map.w3t war3map.w3t";
        if ($? == -1) {
          printf "Unable to spawn MPQEditor process";
        } elsif ($? >> 8 == 5) {
          printf "ERROR: Failed to add icon index files, you may not have valid permissions or are blocked by windows UAC. Ensure map files are not in a UAC protected location %d\n", $? >> 8;
        } elsif ($? >> 8 > 0) {
          printf "ERROR: Unknown. AMAI not have installed correctly. Adding icon index files:%d\n", $? >> 8;
        }
      }
